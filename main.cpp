...
void loop() {
    ...
    switch (currentState) {
        ...
        case Drive:
            /*
                1. Map throttle to power, then power to RPM target
                2. Engage cascading PID with RPM target (return motor pwm output, from -1-1)
                3. dutyCycle = PIDOutput
            */
            // dutyCycle = updatePIDCascading()
            break;
    }
